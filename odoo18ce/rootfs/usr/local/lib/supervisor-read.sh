...
# a budget runs out, flushing the cache keys behind it before each retry so
# the Supervisor is really asked again. The value goes to stdout; an
# exhausted budget is exit 1 with nothing printed, and the caller decides
# what an empty value means and says how long was waited.
#
# Usage:
...
woow::supervisor.read() {
    ...
    while :; do
        ...
        # it": the wait is then never shorter than asked, at most a second
        # longer.
