...
| `G-05` | Ingress-only 且 Supervisor 取不到 LAN 位址（Canonical URL 為空）時，Website 分享 snippet 仍把 `location.href` 交給社群網站，其中含 Supervisor 的 ingress token | **Important** | RC-9 | `nginx.conf.template` Ingress 資產區塊的 `const currentUrl=` 補丁：全域為空時依 ADR 0006 退回今日行為 | 依 ADR 0006「無 Canonical URL 時所有補丁維持現狀」，此形態不變。承接路徑：設定 `public_url`，或從 Public origin 分享。若要在此形態下也去掉 token（例如只剝掉 ingress 前綴、不換基底），需要一份改寫該條的決定。追蹤 #70 |
